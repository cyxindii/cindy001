...
<div class="container my-5">
  ...
  <form action="status.php" method="post">
    ...
    <input type="text" class="form-control" value="<?= $user['role'] === 'teacher' || $user['role'] === 'M' ? '教職員/管理員' : '學生' ?>" readonly /><br/><br/>
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      ...
      $total = ($user['role'] === 'teacher' || $user['role'] === 'M') ? 0 : $dinner;

      // 顯示報名資訊
      echo "<h4 class='mt-4'>報名資訊：</h4>";
      echo "姓名：$name<br/>";
      echo "身分：" . ($role === 'teacher' || $role === 'M' ? '教職員/管理員' : '學生') . "<br/>";
